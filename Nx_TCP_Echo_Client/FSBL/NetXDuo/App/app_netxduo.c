...
static VOID App_TCP_Thread_Entry(ULONG thread_input)
{
  ...
  ULONG google_dns_ip = IP_ADDRESS(8, 8, 8, 8);
  ULONG responder_ip;
  UINT dummy_port;
  int i;

  printf("<Communication Réseau>\r\n");

  for (i = 1; i <= 10; i++)
  {
    /* On utilise GPIO_PIN_7 qui est la LED Bleue sur Nucleo N657 */
    HAL_GPIO_WritePin(GPIOG, GPIO_PIN_7, GPIO_PIN_SET);

    printf("[%d/10] Envoi vers 8.8.8.8... ", i);

    status = nx_icmp_ping(&NetXDuoEthIpInstance, google_dns_ip, "PING", 4, &response_ptr, 200);

    if (status == NX_SUCCESS)
    {
      nx_udp_source_extract(response_ptr, &responder_ip, &dummy_port);
      printf("Réponse de %lu.%lu.%lu.%lu : OK\n",
             (responder_ip >> 24) & 0xFF, (responder_ip >> 16) & 0xFF,
             (responder_ip >> 8) & 0xFF, (responder_ip & 0xFF));

      nx_packet_release(response_ptr);

      /* Flash de la LED VERTE pour le succès */
